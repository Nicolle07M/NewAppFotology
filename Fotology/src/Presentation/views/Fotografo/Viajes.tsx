...
export default function Viajes() {
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        await loadImagesFromStorage('Viajes');
  ...
  const handleEditOption = (index: number) => {
    console.log('Editar');
    if (index !== null) {
      navigation.navigate('PublicacionVScreen', { image: images[index] });
    }
  };
  ...
  return (
    <SafeAreaView style={VistaStyles.container}>
      ...
      <View style={VistaStyles.footer}>
        <TouchableOpacity style={VistaStyles.button} onPress={() => navigation.navigate('PublicacionVScreen')}>

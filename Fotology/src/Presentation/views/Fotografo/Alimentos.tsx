...
export default function Alimentos() {
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        await loadImagesFromStorage('Alimentos');
    ...
    if (newImages && newImages.length > 0) {
      Promise.all(newImages.map(image => uploadImageToCloudinary(image)))
        .then(cloudinaryImages => {
          ...
          saveImagesToStorage('Alimentos', updatedImages);
        })
        .catch(error => {
  ...
  const handleEditOption = (index: number) => {
    console.log('Editar');
    if (index !== null) {
      navigation.navigate('PublicacionAScreen', { image: images[index] });
    }
  };
  ...
  return (
    <SafeAreaView style={VistaStyles.container}>
      ...
      <View style={VistaStyles.footer}>
        <TouchableOpacity style={VistaStyles.button} onPress={() => navigation.navigate('PublicacionAScreen')}>

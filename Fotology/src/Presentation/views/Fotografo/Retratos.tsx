...
export default function Retratos() {
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        await loadImagesFromStorage('Retratos');
    ...
    if (newImages && newImages.length > 0) {
      Promise.all(newImages.map(image => uploadImageToCloudinary(image)))
        .then(cloudinaryImages => {
          ...
          saveImagesToStorage('Retratos', updatedImages);
        })
        .catch(error => {
  ...
  const handleEditOption = (index: number) => {
    console.log('Editar');
    if (index !== null) {
      navigation.navigate('PublicacionRScreen', { image: images[index] });
    }
  };
  ...
  return (
    <SafeAreaView style={VistaStyles.container}>
      ...
      <View style={VistaStyles.footer}>
        <TouchableOpacity style={VistaStyles.button} onPress={() => navigation.navigate('PublicacionRScreen')}>

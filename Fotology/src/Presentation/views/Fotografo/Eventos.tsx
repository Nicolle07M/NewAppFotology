...
export default function Eventos() {
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        await loadImagesFromStorage('Eventos');
    ...
    if (newImages && newImages.length > 0) {
      Promise.all(newImages.map(image => uploadImageToCloudinary(image)))
        .then(cloudinaryImages => {
          ...
          saveImagesToStorage('Eventos', updatedImages);
        })
        .catch(error => {
  ...
  const handleEditOption = (index: number) => {
    console.log('Editar');
    if (index !== null) {
      navigation.navigate('PublicacionEScreen', { image: images[index] });
    }
  };
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              setImages(updatedImages);
              saveImagesToStorage('Viajes', updatedImages);
  ...
  return (
    <SafeAreaView style={VistaStyles.container}>
      ...
      <View style={VistaStyles.footer}>
        <TouchableOpacity style={VistaStyles.button} onPress={() => navigation.navigate('PublicacionEScreen')}>
